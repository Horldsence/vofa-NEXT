...
use provider::{validate_config, AdapterInfo, AiProviderConfig, ToolSpecDto};
use serde::Serialize;
use serde_json::Value;
use session::{derive_history, ChatSession, SessionMeta, SessionStore, ViewItemDto, ViewRoleDto};
...
pub async fn ai_chat_send(
    ...
    let lang = ui_lang.as_deref().map_or(Lang::Zh, Lang::parse);
...
pub fn ai_tool_resolve(
    ...
    // 语句内取走 sender, MutexGuard 不跨后续处理存活
    let tx = state.pending_frontend.lock().remove(&call_id);
    tx.is_some_and(|tx| {
