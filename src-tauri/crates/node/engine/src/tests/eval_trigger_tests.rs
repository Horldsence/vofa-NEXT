...
use kind::{MathOp, StrNumParams, StrOp};
...
fn test_trigger_auto_level_matches_every_active_frame() {
    ...
    let mut eval_with = |v: f32, ts: &mut HashMap<String, trigger::TriggerState>| {
        input_values.clear();
...
fn test_trigger_manual_tracks_prev_no_false_rising_on_mode_switch() {
    ...
    let mut eval_with =
        |g: &CompiledGraph, v: f32, ts: &mut HashMap<String, trigger::TriggerState>| {
            input_values.clear();
            input_values.insert("knob1".to_string(), v);
            let mut out_str = StringValuesMap::default();
            g.evaluate(
