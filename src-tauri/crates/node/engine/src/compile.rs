...
//!    端口域解析 + 边分类 ([`hir::EdgeClass`]), 跨域边报 DomainMismatch
//! 2. 中端: 平面投影 ([`plane`]) — 值平面拓扑序 (f32 ∪ 字符串边,
//!    保证上游 string 节点先于下游 Str 节点求值) + 字节平面 [`BytePlan`];
//!    跨平面不构成循环由 EdgeFiltered 视图结构性保证
//! 3. 后端: [`lower::lower_value_plane`] — 槽位分配 (f32/字符串双 arena)
...
use kind::{DecoderBlockDef, NodeDef, NodeKind};

use eval::CompiledEval;
use hir::{CompileError, EdgeClass, HirEdge, TypedGraph};
