...
//! - **PortDomain DTO**:`kind::PortDomain` 是领域定义来源, 本 enum 为事件契约
//!   提供 serde-friendly 表达; 二者字段含义一一对应
//! - **后端薄壳**:该类型同时被 `engine::errors` 通过 `pub use` 暴露, 保持
//!   `engine::errors::CompileError` 的历史调用面
//!
//! 跨 IPC 错误流:
//! `engine::TypedGraph::build(...) -> Result<_, CompileError>`
//! → `graph::update_tab_graph` 包装为 `ConfigError::GraphCompile(Box<CompileError>)`
//! → `error::AppError::Graph(Boxed)` → JSON 序列化
//! → 前端 `lib/tauri/errorGuidance.ts` 决定用户文案

use serde::{Deserialize, Serialize};

/// 跨 IPC 端口域 DTO — 与 `kind::PortDomain` 含义一致, 提供 serde 派生.
