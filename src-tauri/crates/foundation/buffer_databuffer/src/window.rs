...
impl DataBuffer {
    ...
    pub fn get_window(&self, start_ms: i64, end_ms: i64) -> WaveformWindow {
        ...
        let from_us = window_bound_us(latest_us, start_ms);
        let to_us = window_bound_us(latest_us, end_ms);
    ...
    pub fn get_recent(&self, count: usize) -> WaveformWindow {
        ...
        let rel_ts: Vec<i64> = ts.iter().map(|&t| rel_ms(t, latest_us)).collect();
...
/// 计算窗口边界时间戳: `latest_us + offset_ms*1000`, 下限钳到 0
///
/// 时间戳值域为正数微秒, i64 中转不会溢出 (u64 微秒自 1970 起远小于 i64 上限)
fn window_bound_us(latest_us: u64, offset_ms: i64) -> u64 {
    let latest = i64::try_from(latest_us).unwrap_or(i64::MAX);
    let shifted = (latest + offset_ms.saturating_mul(1000)).max(0);
    u64::try_from(shifted).unwrap_or(0)
}

/// 相对最新时间戳的毫秒数 (负数 = 过去)
fn rel_ms(ts_us: u64, latest_us: u64) -> i64 {
    let ts = i64::try_from(ts_us).unwrap_or(i64::MAX);
    let latest = i64::try_from(latest_us).unwrap_or(i64::MAX);
    ts.saturating_sub(latest) / 1000
}
